...
if(isset($blogData))
{
	...
	$tag = $blogData->tag; 
    $short = $blogData->short_description;
    $description = $blogData->description; 
    $image = $blogData->image;
    $credit = $blogData->credit;
    $credit_url = $blogData->credit_url;
}
        ...
        <div class="row breadcrumbs-top d-inline-block">
          <div class="breadcrumb-wrapper col-12">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item"><a href="{{ route('auth.blog.index') }}">Blogs</a>
              </li>
...
<div class="form-body">
  <h4 class="form-section"><i class="fa fa-eye"></i> About Blogs</h4>
    <div class="row">
      <div class="col-md-12">
              ...
              <div class="col-md-9">
                  <select name="category" class="form-control select2"> 
                        ...
                        @foreach($blog_categories as $s)
                            <option value="{{ $s->id }}" <?php if($s->id == $category){ echo "selected"; } ?>>{{ $s->title }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="col-md-9">
                    <select name="tag" class="form-control select2"> 
                        ...
                        @foreach($blog_tags as $s)
                            <option value="{{ $s->id }}" <?php if($s->id == $tag){ echo "selected"; } ?>>{{ $s->title }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="col-md-12">
            <div class="form-group row">
                ...
                <div class="col-md-9">
                  ...
                  @if(trim($image))
                    <img src="{{ env('APP_URL')}}/storage/app/public/blogs/{{ trim($image) }}" style="width: 100%; height: 300px;">
                  @else
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="form-group row">
          ...
          <div class="col-md-9">
              <input type="text" id="userinput1" class="form-control border-primary" placeholder="Credit" name="credit" value="{{ $credit }}" required="">
            </div>
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col-md-12">
        <div class="form-group row">
          ...
          <div class="col-md-9">
              <input type="text" id="userinput1" class="form-control border-primary" placeholder="Credit URL" name="credit_url" value="{{ $credit_url }}" required="">
            </div>
        </div>
      </div>
    </div>
