...
<div class="app-content content">
	...
	<td>
		@if($u->status == 0)
		    <a href="{{ route('auth.policies.setfeature', $u->id) }}"><span class="badge badge-primary">Set as Featured</span></a>
		@else($u->status == 1)
		    <a href="{{ route('auth.policies.unfeature', $u->id) }}"><span class="badge badge-success">Featured</span></a>
		@endif
	</td>
	<td style="display: flex; border-bottom: none;">
		<a href="{{ route('auth.policies.edit', $u->id) }}" onclick="return confirm('Are you sure to Edit this Policy ?');" data-toggle="tooltip" data-placement="top" title="Edit">
			<button type="button" class="btn btn-primary"><i class="fa fa-pencil"></i></button>
		</a>&nbsp;

        <a href="{{ route('auth.policies.delete', $u->id) }}" onclick="return confirm('Are you sure to Delete this Policy ?');" data-toggle="tooltip" data-placement="top" title="Delete">
